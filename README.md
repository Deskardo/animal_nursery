...
Реализована иерархия классов в соответствии с диаграммой классов
1. Добавление нового животного:
- Используется метод add_animal, внутри метода создаются аргументы
для создания класса, все аргументы должны пройти проверку на корректность ввода
- После добавления аргументов создается экземпляр класса указанного в 
аргументе animal_type с занесеним в список animal_registry

2. Список комманд животного:
- список комманд присваивается при создании экземпляра класса в качестве аттрибута
с помощью метода list_commands выводится список доступных для конкретного экземляра комманд
программа запрашивает у пользователя имя и тип искомого животного и выводит все совпадения в формате
 имя дата рождения список комманд 

3. Обучения новым коммандам:
- После считывания комманд, пользователь может захотеть добавить комманду живаотному
вызывается метод train_commands, вводится имя и дата рождения конкретного экземпляра класса 
(я предположил, что сочетание этих полей может быть уникальным, но в дальнейшем нужно будет использовать ID экземпляра)
и добавляется комманда, в процессе добавления, программа проводит проверку корректности введенных данных.

4. Вывести список животных по дате рождения:
- Сортирует список с использованием анонимной функции в качестве компаратора
Выводит в формате тип имя дата рождения комманды

5. Реализовано консольное меню для вызова методов:
~~~
--- Animal Registry Menu ---
1. Add new animal
2. List commands for an animal
3. Train new commands for an animal
4. List animals by birth date
5. Show count of animals
6. Exit
~~~

6. Счетчик экземляров добавлен в родительский класс Animal
- Счетчик увеличивается при каждом добалвении нового экземпляра.